...
public sealed class CdController
{
    ...
    {
        // CD sectors arrive independently of the GPU frame cadence. Early
        // boot loaders can stream before their first VSync, so advance a
        // pending read when the emulated CPU next observes the controller.
        // Frame-driven callers still use AdvanceStreaming from PresentFrame.
        if (_reading && _streamPending && _irqFlags == 0 &&
            _pendingIrqs.Count == 0)
            AdvanceStreaming();
    ...
    private void ExecuteCommand(byte cmd)
    {
        ...
        switch (cmd)
        {
            ...
            case 0x0E: // set mode
                QueueIrq(3, [DriveStatus()]);
                break;
            case 0x13: // GetTN: first and last track numbers are BCD.
                QueueIrq(3,
                [
                    DriveStatus(),
                    ToBcd(_fs.FirstTrackNumber),
                    ToBcd(_fs.LastTrackNumber),
                ]);
                break;
            case 0x14: // GetTD: absolute track start (or lead-out) as BCD MSF.
            {
                int track = prms.Count > 0 ? FromBcd(prms[0]) : 0;
                int lba;
                bool found = track == 0
                    ? (lba = _fs.LeadOutLba) >= 0
                    : _fs.TryGetTrackStartLba(track, out lba);
                if (!found)
                {
                    QueueIrq(5, [DriveStatus(), 0x10]);
                    break;
                }
                int absolute = checked(lba + 150);
                QueueIrq(3,
                [
                    DriveStatus(),
                    ToBcd(absolute / (60 * 75)),

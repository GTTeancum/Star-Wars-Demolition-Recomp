...
internal static unsafe class InputManager
{
    ...
    static string? _ignoreAbsoluteAfterStage;
    ...
    static void InitializeConfiguration()
    {
        ...
        _scriptExitAfterStageVisits =
            ...
            parsedStageVisits > 0
                ? parsedStageVisits
                : -1;
        string? ignoreAbsoluteAfterStage =
            Environment.GetEnvironmentVariable(
                "RECOMPONE_SCRIPT_IGNORE_ABSOLUTE_AFTER_STAGE");
        _ignoreAbsoluteAfterStage = string.IsNullOrWhiteSpace(
                ignoreAbsoluteAfterStage)
            ? null
            : NormalizeStage(ignoreAbsoluteAfterStage);
        ParseScriptedInput();
    ...
    static void ApplyScriptedInput()
    {
        ...
        {
            if (pulse.Stage == null &&
                _ignoreAbsoluteAfterStage != null &&
                string.Equals(
                    _scriptStage,
                    _ignoreAbsoluteAfterStage,
                    StringComparison.OrdinalIgnoreCase))
                continue;
            int currentPoll;
            if (pulse.Stage == null)

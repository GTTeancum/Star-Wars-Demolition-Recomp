...
public static class LibCd
{
    ...
    static int _demolitionFileStartLba = -1;
 ...
 => c.V0 = (uint)SyncResult(m, c.A1);
    ...
    static void CdResetState()
    {
        ...
        _cdMixLr = _cdMixRl = 0;
        _cdMuted = false;
        _v8FileStartLba = _v82FileStartLba = _demolitionFileStartLba = -1;
        Array.Clear(_pos);
    ...
    public static void WaitForDemolitionSector(CpuContext c, IMemory m)
    {
        uint previous = m.ReadU32(c.GP + 0xCC4u);
        uint producer = m.ReadU32(c.GP + 0xCC8u);
        for (int attempt = 0; producer == previous && attempt < 8; attempt++)
        {
            ServiceReadOnce();
            producer = m.ReadU32(c.GP + 0xCC8u);
        }

        m.WriteU32(c.GP + 0xCC4u, producer);
        c.V0 = previous;
    }
    ...
    public static void BeginDemolitionFileRead(CpuContext c, IMemory m)
    {
        _demolitionFileStartLba = unchecked((int)c.A0);
    }
    ...
    public static void ReadDemolitionFileBytes(CpuContext c, IMemory m)
    {
        ReadFileBytes(
            c, m, _demolitionFileStartLba, c.GP + 0xCCCu,
    ...
    public static void SeekDemolitionFile(CpuContext c, IMemory m)
    {
        SeekFile(c, m, c.GP + 0xCCCu);
    }

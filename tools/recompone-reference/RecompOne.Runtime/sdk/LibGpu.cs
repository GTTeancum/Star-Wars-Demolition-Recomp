...
public static class LibGpu
{
    ...
    static int _demolitionOtTraceCount;
    static readonly bool TraceDemolitionOt =
        Environment.GetEnvironmentVariable("RECOMPONE_TRACE_DEMOLITION_OT") == "1";

    public static void LoadImage(CpuContext c, IMemory m)
    {
        var gpu = Runtime.Gpu;
        uint rect = c.A0;
        int x = S16(m, rect + 0x00u);
        int y = S16(m, rect + 0x02u);
        ...
        uint words = ((uint)w * (uint)h + 1u) / 2u;
        for (uint i = 0; i < words; i++)
        {
            uint source = c.A1 + i * 4u;
    ...
    public static void StoreImage(CpuContext c, IMemory m)
    {
        ...
        // Demolition performs many single-pixel visibility probes while it is
        // building a gameplay frame. A synchronous Enhanced-GL readback for
        // each probe stalls the render thread for seconds. The software shadow
        // is sufficient for these best-effort probes and keeps the simulation
        // responsive; bulk shell/loading readbacks retain the exact GPU path.
        if (GpuHle.GameplayActive && (long)w * h <= 64)
        {
            uint pixels = (uint)w * (uint)h;
            for (uint pixel = 0; pixel < pixels; pixel += 2u)
            {
                int px0 = (x + (int)(pixel % (uint)w)) & (Gpu.VramWidth - 1);
                int py0 = (y + (int)(pixel / (uint)w)) & (Gpu.VramHeight - 1);
                ushort lo = gpu.Vram[py0 * Gpu.VramWidth + px0];
                ushort hi = 0;
                if (pixel + 1u < pixels)
                {
                    uint next = pixel + 1u;
                    int px1 = (x + (int)(next % (uint)w)) & (Gpu.VramWidth - 1);
                    int py1 = (y + (int)(next / (uint)w)) & (Gpu.VramHeight - 1);
                    hi = gpu.Vram[py1 * Gpu.VramWidth + px1];
                }
                m.WriteU32(c.A1 + pixel * 2u, (uint)(lo | (hi << 16)));
            }
            c.V0 = 0u;
            return;
        }
    ...
    public static void ClearImage(CpuContext c, IMemory m)
    {
        ...
        uint colour = ((c.A3 & 0xFFu) << 16) |
                      ((c.A2 & 0xFFu) << 8) |
                      (c.A1 & 0xFFu);
        gpu.WriteGp0(0x02000000u | colour);
        gpu.WriteGp0(((uint)(ushort)y << 16) | (ushort)x);
        gpu.WriteGp0(((uint)(ushort)h << 16) | (ushort)w);
        c.V0 = 0u;
    }
    ...
    public static void DrawOTag(CpuContext c, IMemory m)
    {
        ...
        int packetCount = 0;
        int wordCount = 0;
        var opcodes = TraceDemolitionOt && GpuHle.GameplayActive
        ...
        for (int guard = 0; guard < 0x100000; guard++)
        {
            ...
            {
                packetCount++;
                wordCount += checked((int)count);
                if (opcodes != null)
                {
                    byte opcode = (byte)(m.ReadU32(addr + 4u) >> 24);
        ...
        if (opcodes != null && _demolitionOtTraceCount++ < 16)
            Console.Error.WriteLine(
                $"[DemolitionOT] head=0x{c.A0:X8} buckets={bucketCount} packets={packetCount} " +
                $"words={wordCount} ops={string.Join(',', opcodes.OrderBy(p => p.Key).Select(p => $"{p.Key:X2}:{p.Value}"))}");
    }
        ...
        out string reason)
    {
        ...
        while (offset < wordCount)
        {
            ...
            if (opcode is >= 0xA0 and <= 0xBF)
            {
                if (offset + 3u > wordCount)
                {
                    reason = "truncated image upload header";
                    return false;
                }
                uint size = m.ReadU32(address + 4u + (offset + 2u) * 4u);
                uint width = size & 0xFFFFu;
                uint height = size >> 16;
                if (width == 0u) width = 0x400u;
                else if ((width &= 0x3FFu) == 0u) width = 0x400u;
                if (height == 0u) height = 0x200u;
                else if ((height &= 0x1FFu) == 0u) height = 0x200u;
                uint uploadLength = 3u + (width * height + 1u) / 2u;
                if (offset + uploadLength > wordCount)
                {
                    reason =
                        $"truncated image upload needs={uploadLength} " +

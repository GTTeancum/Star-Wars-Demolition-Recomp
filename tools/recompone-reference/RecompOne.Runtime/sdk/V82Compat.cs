...
public static class V82Compat
{
    ...
    static readonly Stack<uint> NativeSkyRenderScopes = [];
    static readonly Stack<uint> NativeWaterBaseRenderScopes = [];
    static readonly Stack<uint> NativeWaterSurfaceRenderScopes = [];
    ...
    static uint _geometryCompletionGpOffset;
    ...
    const uint V82TerrainTextureTable = 0x800B7270u;
    const uint DemolitionTerrainTextureTable = 0x800C7648u;
    const uint V82TerrainPageTable = 0x800B93F0u;
    ...
    static uint TerrainPageTable => IsDemolition
        ? DemolitionTerrainPageTable
        : V82TerrainPageTable;
    ...
    public static void DemolitionMalloc(CpuContext c, IMemory m)
    {
        m = Dispatcher.UnwrapMemory(m);
        EnsurePcHeap();
        uint request = c.A0;
        ...
        c.V0 = 0u;
        Console.Error.WriteLine(
            $"[DemolitionCompat] PC heap exhausted allocating {request} bytes from 0x{c.RA:X8}");
    }

    public static void DemolitionFree(CpuContext c, IMemory m)
    {
        uint pointer = c.A0;
        if (pointer == 0u) return;
        if (!PcAllocations.Remove(pointer, out var allocation))
        {
            Console.Error.WriteLine(
                $"[DemolitionCompat] ignored non-PC free 0x{pointer:X8} from 0x{c.RA:X8}");
            return;
        }
        InsertFreeBlock(allocation.Header, allocation.Size);
    }

    public static void DemolitionRealloc(CpuContext c, IMemory m)
    {
        m = Dispatcher.UnwrapMemory(m);
        uint pointer = c.A0;
        uint request = c.A1;
    ...
    public static void RunDemolitionLoadVlc(CpuContext c, IMemory m)
        => RunVlcRegion(
            c, m, 0x80108E74u, 0x80109380u, "DEMOLITION LOAD");
    ...
    public static void EndDemolitionObjectRender(CpuContext c, IMemory m)
    {
        if (ObjectRenderScopes.Count == 0)
            return;
        ...
        m = Dispatcher.UnwrapMemory(m);
        ObjectRenderScope scope = ObjectRenderScopes.Pop();
        uint packetEnd = m.ReadU32(c.GP + 0x5E8u);
        ...
        GpuHle.RegisterPacketOwnerRange(
            scope.PacketStart,
            packetEnd,
            $"demolition-object=0x{scope.ObjectAddress:X8}");
    }
    ...
    {
        NativeSkyRenderScopes.Push(NativePacketWriteCursor(c, m));
        BeginNamedRender(c, m, "sky-80050B38");
    }
    ...
    public static void EndSceneryPass(CpuContext c, IMemory m)
    {
        ...
        if (NativeSkyRenderScopes.Count == 0)
            return;
        uint start = NativeSkyRenderScopes.Pop();
        uint end = NativePacketWriteCursor(c, m);
        if (end <= start)
            return;
        GpuHle.RegisterSkyPacketRange(start, end);
        GpuHle.RegisterPacketOwnerRange(start, end, "v82-sky=80050B38");
    }
    ...
    static GpuHle.TerrainQuadDistanceColors
        ReadDreamcastTerrainDistanceColors(
            ...
            uint z)
    {
        // This optional path reconstructs the Dreamcast port's terrain mesh
        // and colour ramp from V8:2-specific state. Demolition shares the PS1
        // packet writers but not those Dreamcast globals; retain its authored
        // packet geometry instead of manufacturing vertices from unrelated
        // memory.
        if (IsDemolition ||
            !ConfigManager.View.HighResolution3D ||
            !IsMaximumLevelOfDetail())
    ...
    public static bool EnterGeometry22164(CpuContext c, IMemory m)
    {
        ...
        return EnterGeometryContinuation(0x80022164u, 0x610u);
    }

    public static bool EnterGeometry22910(CpuContext c, IMemory m)
    {
        CaptureGeometryContinuation(c, m, 0x80022910u);
        return EnterGeometryContinuation(0x80022910u, 0x610u);
    }

    // Star Wars: Demolition uses the same tail-dispatched geometry microcode
    // pattern as V8:2, at a later address and with a different primitive
    // cursor global. Keep its long packet streams on the same iterative host
    // trampoline.
    public static bool EnterDemolitionGeometry23C64(CpuContext c, IMemory m)
    {
        return EnterGeometryContinuation(0x80023C64u, 0x5E8u);
    }

    public static bool EnterDemolitionGeometry23C5C(CpuContext c, IMemory m) =>
        EnterGeometryContinuation(0x80023C5Cu, 0x5E8u);

    public static void LeaveDemolitionGeometryContinuation(
        CpuContext c, IMemory m) => LeaveGeometryContinuation(c, m);

    public static void SkipDemolitionRecursivePrimitive(
        CpuContext c, IMemory m)
    {
        c.S3 += 0x18u;
        Dispatcher.Call(c, m, 0x80023C64u);
    }
    ...
    static bool EnterGeometryContinuation(uint target, uint completionGpOffset)
    {
        ...
        _geometryCompletionGpOffset = completionGpOffset;
        return true;
    }
    ...
    public static void LeaveGeometryContinuation(CpuContext c, IMemory m)
    {
        ...
        try
        {
            ...
            {
                int continuationLimit = _geometryCompletionGpOffset == 0x5E8u
                    ? 2048
                    : 8192;
                int iteration = ++_geometryContinuationIterations;
                if (_geometryCompletionGpOffset == 0x5E8u && iteration > 1 &&
                    (iteration & (iteration - 1)) == 0)
                {
                    Console.Error.WriteLine(
                        $"[DemolitionGeometry] continuation={iteration} " +
                        $"target=0x{_geometryNextTarget:X8} s3=0x{c.S3:X8} " +
                        $"s5={c.S5} s2=0x{c.S2:X8}");
                }
                if (iteration > continuationLimit)
                {
                    if (_geometryClipCount++ < 16)
                        Console.Error.WriteLine(
    ...
    static void CompleteGeometryFrame(CpuContext c, IMemory m)
    {
        ...
        m.WriteU32(c.GP + _geometryCompletionGpOffset, c.S2);
    ...
    // Demolition's shell uses the same asynchronous DrawOTag completion
    // pattern with its callback pointer at gp+C38 and completion flag at
    // gp+5EC. The host has already consumed the ordering table synchronously,
    // so deliver the registered callback before the retail busy-wait.
    public static void ServiceDemolitionDrawSyncWait(CpuContext c, IMemory m)
    {
    ...
    {
        uint view = c.GP + 0xE70u;
        for (int register = 0; register < 5; register++)
            RecompOne.Runtime.Gte.WriteControl(
                register, m.ReadU32(view + (uint)(register * 4)));

        // The original tail explicitly clears translation before object
        // callbacks supply their model-relative coordinates.
        RecompOne.Runtime.Gte.WriteControl(5, 0u);
        RecompOne.Runtime.Gte.WriteControl(6, 0u);
        RecompOne.Runtime.Gte.WriteControl(7, 0u);

        uint light = c.GP + 0x6F8u;
        uint colour = c.GP + 0x718u;
        for (int register = 0; register < 5; register++)
        {
            RecompOne.Runtime.Gte.WriteControl(
                8 + register, m.ReadU32(light + (uint)(register * 4)));
            RecompOne.Runtime.Gte.WriteControl(
                16 + register, m.ReadU32(colour + (uint)(register * 4)));
        }

        RecompOne.Runtime.Gte.WriteControl(13, (uint)m.ReadU8(c.GP + 0xD54u) << 4);
        RecompOne.Runtime.Gte.WriteControl(14, (uint)m.ReadU8(c.GP + 0xD55u) << 4);
        RecompOne.Runtime.Gte.WriteControl(15, (uint)m.ReadU8(c.GP + 0xD56u) << 4);
        RecompOne.Runtime.Gte.WriteControl(21, (uint)m.ReadU8(c.GP + 0xCF0u) << 4);
        RecompOne.Runtime.Gte.WriteControl(22, (uint)m.ReadU8(c.GP + 0xCF1u) << 4);
        RecompOne.Runtime.Gte.WriteControl(23, (uint)m.ReadU8(c.GP + 0xCF2u) << 4);
    }

    // func_800350B0 walks the arena's four-kind visibility tree. Its retail
    // recursion is shallow but extremely hot; direct managed recursion also
    // shares the mutable CpuContext, turning a single first-frame traversal
    // into minutes of host work. Preserve the exact signed split tests and
    // leaf callback while using an explicit host stack. Corrupt cyclic nodes
    ...
    public static void WalkDemolitionVisibilityTree(CpuContext c, IMemory m)
    {
        ...
        int maxX = (int)c.A2;
        int minZ = (int)c.A3;
        int maxZ = (int)m.ReadU32(c.SP + 0x10u);
        var caller = c.Snapshot();
        var pending = new Stack<uint>();
        var visited = new HashSet<uint>();
        pending.Push(root);

        const int nodeLimit = 0x20000;
        int nodeCount = 0;
        while (pending.Count != 0 && nodeCount++ < nodeLimit)
        {
            uint node = pending.Pop();
            uint physical = MemoryMap.ToPhysical(node);
            ...
            if (kind == 0u)
            {
                c.Restore(caller);
                c.A0 = node + 4u;
                Dispatcher.Call(c, m, 0x80034DECu);
                continue;
            }

            uint split = m.ReadU32(node + 4u);
            uint first = m.ReadU32(node + 8u);
            uint second = m.ReadU32(node + 0xCu);
            switch (kind)
            {
                case 1u:
                    // Push the second branch first to retain retail's
                    // first-then-second traversal order.
                    if ((int)split < maxX && second != 0u)
                        pending.Push(second);
                    if (minX < (int)split && first != 0u)
                        pending.Push(first);
                    break;
                case 2u:
                    if ((int)split < maxZ && second != 0u)
                        pending.Push(second);
                    if (minZ < (int)split && first != 0u)
                        pending.Push(first);
                    break;
                case 3u:
                    if (second != 0u)
        ...
        if (pending.Count != 0)
            Console.Error.WriteLine(
                $"[DemolitionVisibility] stopped malformed tree at {nodeLimit} nodes");
    }
    ...
    static int _demolitionVisibilityTraceCount;
    ...
    public static void DrawDemolitionVisibilityLeaf(CpuContext c, IMemory m)
    {
        ...
        c.Restore(caller);
        if ((malformed || linkCount >= linkLimit) &&
            _demolitionVisibilityCycleLogs++ < 16)
            Console.Error.WriteLine(
                $"[DemolitionVisibility] stopped cyclic leaf after {linkCount} links");
    }

    static bool _demolitionGameplayStarted;

    public static bool BeginDemolitionGameplayFrame(CpuContext c, IMemory m)
    {
        GpuHle.GameplayActive = true;
        // Demolition normally receives the vertical-blank interrupt between
        // simulation and this world-render boundary. Static execution has no
        // asynchronous R3000 thread, so present the completed back buffer and
        ...
        if ((GpuHle.DebugGameplayTick % 120) == 0)
            Console.Error.WriteLine(
                $"[DemolitionCompat] gameplay tick={GpuHle.DebugGameplayTick}");
        if (!_demolitionGameplayStarted)
        {
            _demolitionGameplayStarted = true;
            InputManager.SignalScriptStage("gameplay", captureDelayPolls: 30);
            Console.Error.WriteLine(
    ...
    // Static execution builds Demolition's world ordering table correctly,
    // but the later viewport compositor only submits its small stack-local
    // HUD table through the translated DrawOTag path. Submit the retail world
    ...
    // Off. This submitted the world ordering table as soon as the world
    // renderer at 0x80035930 returned, but the engine keeps adding to that same
    // table afterwards - the HUD, and the pause, objectives and quit overlays.
    // Drawing it early meant everything added later was never submitted, which
    // is why gameplay had no radar, armour ring or reticle and why pausing
    // froze the world with no menu on top of it. The native end-of-frame
    // submission already draws the complete table, including the world, so the
    // early call was redundant as well as destructive.
    //
    // Set RECOMPONE_DEMOLITION_SUBMIT_WORLD_OT=1 to restore the old behaviour
    // for comparison.
    static readonly bool SubmitWorldOrderingTable =
        Environment.GetEnvironmentVariable(
            "RECOMPONE_DEMOLITION_SUBMIT_WORLD_OT") == "1";
        ...
        IMemory m)
    {
        ...
        try
        {
            uint orderingTable = m.ReadU32(c.GP + 0xC40u);
            if (orderingTable == 0u)
                return;
            c.A0 = orderingTable + 0x3FFCu;
    ...
    public static void WaitDemolitionCardOperation(CpuContext c, IMemory m)
    {
        const uint state = 0x80114810u;
        const uint service = 0x801125C8u;
        uint nonblocking = c.A0;
        uint result1 = c.A1;
        uint result2 = c.A2;
